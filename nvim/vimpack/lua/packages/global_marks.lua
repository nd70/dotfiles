vim.pack.add({
	"https://github.com/nd70/global_marks.nvim",
})

local ok, gm = pcall(require, "global_marks")
if not ok then
	return
end

gm.setup()

vim.keymap.set("n", "m", function()
	local ok_char, ch = pcall(vim.fn.getcharstr)
	if not ok_char or not ch or ch == "" then
		return
	end

	local mark = ch:sub(1, 1)

	-- perform the real mark set
	vim.cmd.normal({ args = { "m" .. mark }, bang = true })

	-- notify global_marks explicitly
	local ok_mod, gm_mod = pcall(require, "global_marks")
	if ok_mod and gm_mod and type(gm_mod.on_mark_set) == "function" then
		gm_mod.on_mark_set(mark)
	end
end, {
	noremap = true,
	silent = true,
	desc = "Set mark and update global_marks",
})

vim.keymap.set("n", "<leader>mc", "<cmd>GMarkClear<CR>", { desc = "Clear all marks" })
vim.keymap.set("n", "<leader>ml", "<cmd>GMarksList<CR>", { desc = "Show all marks" })

do
	local m_mapped = false
	for _, mp in ipairs(vim.api.nvim_get_keymap("n")) do
		if mp.lhs == "M" then
			vim.notify("global_marks: 'M' is already mapped; not replacing it", vim.log.levels.INFO)
			m_mapped = true
			break
		end
	end

	if not m_mapped then
		vim.keymap.set("n", "M", function()
			local ok_char, ch = pcall(vim.fn.getcharstr)
			if not ok_char or not ch or ch == "" then
				return
			end
			local mark = ch:sub(1, 1)

			local loaded_mod, gm_mod = pcall(require, "global_marks")
			if loaded_mod and gm_mod and type(gm_mod.jump) == "function" then
				pcall(gm_mod.jump, mark)
			else
				pcall(vim.cmd, "GMarkJump " .. mark)
			end
		end, {
			noremap = true,
			silent = true,
			desc = "Jump to global mark (M{char})",
		})
	end
end

vim.keymap.set("n", "<leader>md", function()
	local ok_mod, gm_mod = pcall(require, "global_marks")
	if not ok_mod or not gm_mod or type(gm_mod.marks) ~= "table" then
		vim.notify("global_marks not loaded", vim.log.levels.WARN)
		return
	end

	local bufnr = vim.api.nvim_get_current_buf()
	local row = vim.api.nvim_win_get_cursor(0)[1]

	local to_delete = {}
	for mark, info in pairs(gm_mod.marks) do
		if mark:match("%l") then
			local tbl = info
			if tbl and tbl[tostring(bufnr)] then
				local ent = tbl[tostring(bufnr)]
				if ent and ent.row == row then
					table.insert(to_delete, mark)
				end
			end
		else
			local ent = info
			if ent and ent.bufnr == bufnr and ent.row == row then
				table.insert(to_delete, mark)
			end
		end
	end

	if #to_delete == 0 then
		vim.notify("No mark on this line", vim.log.levels.INFO)
		return
	end

	local deleted = 0
	for _, mark in ipairs(to_delete) do
		local ok2, gm2 = pcall(require, "global_marks")
		if ok2 and gm2 and type(gm2.delete) == "function" then
			pcall(gm2.delete, mark)
			deleted = deleted + 1
		else
			local suc = pcall(vim.cmd, "GMarkDel " .. mark)
			if suc then
				deleted = deleted + 1
			end
		end
	end

	vim.notify(("Deleted %d mark(s)"):format(deleted), vim.log.levels.INFO)
end, {
	noremap = true,
	silent = true,
	desc = "Delete global mark(s) on the current line",
})
